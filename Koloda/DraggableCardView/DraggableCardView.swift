...
import UIKit
// import pop  // 已移除，改用 UIKit 动画
// 注意：POP 兼容性定义在 POPCompatibility.swift 中
...
var cardSwipeActionAnimationDuration: TimeInterval = DragSpeed.default.rawValue
...
public class DraggableCardView: UIView, UIGestureRecognizerDelegate {
    ...
    private func swipeAction(_ direction: SwipeResultDirection) {
        ...
        translationAnimation?.completionBlock = { (_ animation: UIKitBasicAnimation?, _ finished: Bool) in
    ...
    func swipe(_ direction: SwipeResultDirection, completionHandler: @escaping () -> Void) {
        if !dragBegin {
            ...
            swipePositionAnimation?.completionBlock = { (_ animation: UIKitBasicAnimation?, _ finished: Bool) in
                self.removeFromSuperview()
                completionHandler()
            }
